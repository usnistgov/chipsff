...
class MaterialsAnalyzer:
    ...
    def run_phonon_analysis(self, relaxed_atoms):
        ...
        self.log(f"Converting frequencies in {band_yaml_filepath} to cm^-1 while preserving formatting...")
        yaml = YAML()
        ...
        self.log(f"Frequencies in {band_yaml_filepath} converted to cm^-1 with formatting preserved")
        # --- End post-processing ---
        ...
        for ii, k in enumerate(kpoints.kpts):
            ...
            if ii == 0 or k_str != ",".join(map(str, kpoints.kpts[ii - 1])):
                ...
                eigenvalues.append((k, freqs_at_k_cm))  # Store frequencies in cm^-1
        ...
        freqs_dos = np.array(tdos.frequency_points) * THz_to_cm  # Convert to cm^-1
        dos_values = tdos.dos
    ...
    def run_all(self):
        ...
        if 'analyze_surfaces' in self.properties_to_calculate:
            ...
            for indices in indices_list:
                ...
                try:
                    ...
                    matching_entry = next(
                        (entry for entry in surface_entries if entry['name'].strip() == surface_name.strip()),
                        None
                    )
        ...
        if 'analyze_defects' in self.properties_to_calculate:
            ...
            final_results['vacancy_energy'] = [
                ...
                for ve_name, ve, vee in zip(
                    [f"{self.jid}_{defect.to_dict()['symbol']}" for defect in Vacancy(self.atoms).generate_defects(
        ...
        # Additional analyses
        if 'analyze_interfaces' in self.properties_to_calculate and self.film_jid and self.substrate_jid:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_file", default="input.json", type=str, help="Path to the input configuration JSON file")
